      ...
      <div class="modal-content">
        ...
        <div v-else class="modal-body">
          ...
          <form class="form-horizontal" v-on:submit.prevent="$emit('modal-save')">
            ...
            >
              <label class="col-sm-3 control-label" v-bind:for="id + '-ni'">
                {{ $t('proxypass.name') }}
                <doc-info
                  :placement="'right'"
            ...
            <div v-bind:class="['form-group', vErrors.Target ? 'has-error' : '']">
              <label
                class="col-sm-3 control-label"
                v-bind:for="id + '-Target'"
              >{{ $t('proxypass.Target') }}</label>
              <div class="col-sm-9">
            ...
            <div v-bind:class="['form-group', vErrors.Description ? 'has-error' : '']">
              <label
                class="col-sm-3 control-label"
                v-bind:for="id + '-di'"
              >{{ $t('proxypass.description_label') }}</label>
              <div class="col-sm-9">
                <input
                  type="text"
                  v-model="Description"
            ...
            <div v-if="advanced">
              ...
              <div v-bind:class="['form-group', vErrors.ValidFrom ? 'has-error' : '']">
                <label
                  class="col-sm-3 control-label"
                  for="textInput-modal-markup"
                >{{$t('proxypass.ValidFromCIDR')}}</label>
                <div class="col-sm-9">
                  <textarea
                    v-bind:id="id + '-di'"
                    v-model="ValidFrom"
                    class="form-control"
                    :placeholder="$t('proxypass.ValidFromCIDR_help')"
                  ></textarea>
              ...
              >
                ...
                >{{$t('proxypass.SSL/TLS_certs')}}</label>
                <div class="col-sm-9">
                  <select
                    type="text"
              ...
              <!-- SSL forced -->
              <div v-bind:class="['form-group', vErrors.HTTP ? 'has-error' : '']">
                <label
                  class="col-sm-3 control-label"
                  v-bind:for="id + '-HTTP'"
                >{{$t('proxypass.ForceHTTPS')}}</label>
                <div class="col-sm-9">
                  <input
                    type="checkbox"
                    true-value="no"
              ...
              >
                ...
                >{{$t('proxypass.AcceptInvalidCertificateFromTarget')}}</label>
                <div class="col-sm-9">
                  <input
                    type="checkbox"
                    true-value="yes"
                    false-value="no"
                    v-model="CertVerification"
                    class="form-control"
                  >
                  <span
                    v-if="vErrors.CertVerification"
                    class="help-block"
                  >{{ vErrors.CertVerification }}</span>
                </div>
              </div>
              ...
              >
                ...
                >{{$t('proxypass.PreserveHostHeaderToTarget')}}</label>
                <div class="col-sm-9">
                  <input
                    type="checkbox"
                    true-value="yes"
                    false-value="no"
                    v-model="PreserveHost"
                    class="form-control"
                  >
